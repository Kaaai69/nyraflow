...
export interface CoverflowCarouselProps {
  ...
  cardWidth?: string;
  /** Card width ÷ height. 1 is the square original; 1.6 is a 16:10 screenshot. */
  cardAspect?: number;
  ...
  label?: string;
  /** Fires whenever a different card reaches the centre. */
  onSelectedChange?: (index: number) => void;
  /** Fires on a click of the card that is already centred, and on Enter. */
...
export function CoverflowCarousel({
  ...
  /** Survives the pointer-up so the click that follows a drag can be ignored. */
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A few pixels of slop, so a click with an unsteady hand still counts as
    // a click rather than a one-pixel throw.
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A tap, not a throw. Pointer capture sends every event to the frame, so a
    // click listener on the card itself never fires — hit-test the point instead.
    // The plural call matters: inside the preserve-3d stage the topmost hit is
    // the stage itself, and the card sits further down the same stack.
    if (!drag.moved) {
      const stack = document.elementsFromPoint(event.clientX, event.clientY);
      const card = stack
        .map((node) => node.closest?.("[data-cf-index]"))
        .find(Boolean);
      if (card) {
        const index = Number(card.getAttribute("data-cf-index"));
        // A side card comes to the centre; the centre card is the one that opens.
  ...
  return (
    ...
    >
      ...
      {showCaption && active?.title && (
        <div
          key={selected}
          className="mt-2 flex animate-cf-fade-in flex-col items-center px-6"
        >
